...
$.extend({
    CourierCalcAjax: function( _rid )
    {
        var _checks = $('.courier-pay:checked');
        var _row = $('.priceElement').closest('.row');
        
        if( _checks.length > 0 )
        {
            $.ajax({
                type: 'POST',
                data: 'ajax=click&do=calc&rid='+_rid+'&'+_checks.serialize(),
                url: location.href,
                cache: false,
                processData: false,
                beforeSend: function(){
                ...
                success: function(data){
                    ...
                    if( data != '' )
                    {
                        ...
                        if( typeof(obj.price) != "undefined" && obj.price !== null )
                        {
                            if( obj.price != '' )
                            {
                                $('.weightElement').html( obj.weight ).show();
                                $('.priceElement').html( obj.price ).show();
                                $('.addPriceElement').remove(); 
                                if( obj.addLabel != '' )
                                   $( obj.addLabel ).insertBefore( _row );
                            }
                            else
                            {
                                $('.weightElement').html( '' ).hide();
                                $('.priceElement').html( '' ).hide();
                                $('.addPriceElement').remove();    
                            }
                        }
                    }
...
$(document).ready(function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if( data != '' )
                    {
                        ...
                        $( '#procedure-form #Orders_total_price' ).attr('readonly','readonly');
    ...
    $(document).on('keyup', '#procedure-form #Orders_price', function(){
        ...
        if( _v != '' && $.isNumeric(_v) )
        {
            ...
            _t.val( totalVal.toFixed(2) ).attr('readonly','readonly');
        }
        else
        {
            _t.val( '' );
    ...
    $(document).on('click', '.calculateButton', function(e) {
        ...
        var _liquid = $('#liquid').prop('checked');
        var _p = 0;
        ...
        _label.find('span').html(' '+  ( _p / _currency ).toFixed(2) + ' USD');
        _label.show();
    ...
    $(document).on('click', '.courier-pay', function(e){
        var _this = $( this );
            
        var _rid = $('#CourierOrders_addr_id').val();
        
        $.CourierCalcAjax( _rid );
    });
    
    $(document).on('change', '#CourierOrders_addr_id', function(e){
        var _this = $( this );
        
        $.CourierCalcAjax( _this.val() );
    });
    
    $(document).on('click', '.og-courier', function(e) {
        var _this = $(this);
        var _b = $('.og-courier-pay');

        if( $('.og-courier:checked').length > 0 )
            _b.show();
        else
            _b.hide();
    });
    
    $(document).on('click', '.og-courier-pay', function(e) {
        var _this = $(this);
        var _c = $('.og-courier:checked');
        if( _c.length > 0 )
            window.location.href = _this.data('url')+'?'+_c.serialize()
